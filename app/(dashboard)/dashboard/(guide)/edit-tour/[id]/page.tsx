...
export default function EditTourPage() {
  ...
  const { data: myToursData, isLoading: toursLoading } =
    useGetGuideMyToursQuery({});
  ...
  const tours: Tour[] = Array.isArray(myToursData)
    ? myToursData
    : myToursData?.data || [];
  const tour = tours.find((t) => t._id === tourId);
  const categories = enumsData?.data?.categories || [
    "FOOD",
    "HISTORICAL",
    "ART",
    "NATURE",
  ...
  useEffect(() => {
    if (tour && categories.length > 0 && !isFormInitialized) {
      ...
      setImportantInfo(
        tour.importantInfo?.length > 0 ? tour.importantInfo : [""]
      );
      ...
      setTimeout(() => {
        ...
        setValue("status", tour.status || "ACTIVE", {
          shouldValidate: true,
          shouldDirty: false,
        });
  ...
  const onSubmit = async (data: TourFormData) => {
    try {
      ...
      const cleanImportantInfo = importantInfo.filter(
        (info) => info && info.trim() !== ""
      );
      ...
      if (
        JSON.stringify(cleanHighlights) !== JSON.stringify(originalHighlights)
      ) {
      ...
      if (
        JSON.stringify(cleanNotIncluded) !== JSON.stringify(originalNotIncluded)
      ) {
        formData.append("notIncluded", JSON.stringify(cleanNotIncluded));
      }
      if (
        JSON.stringify(cleanImportantInfo) !==
        JSON.stringify(originalImportantInfo)
      ) {
      ...
      toast.error(
        error?.data?.message || "Failed to update tour. Please try again."
      );
    }
  };
      ...
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-md p-4">
          ...
          <p className="text-red-600 text-sm mt-1">
            The tour you're trying to edit doesn't exist or you don't have
                      ...
                      <SelectContent>
                        {enumsData?.data?.statuses?.map((stat: string) => (
                          <SelectItem key={stat} value={stat}>
                            {stat.charAt(0) +
                              stat.slice(1).toLowerCase().replace(/_/g, " ")}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <div className="space-y-4">
                {itineraryFields.map((field, index) => (
                  ...
                  >
                    ...
                    <div className="mt-4">
                      <Label
                        htmlFor={`itinerary-location-${index}`}
                        className="text-sm font-medium text-gray-700"
                      >
                        Location (optional)
                      </Label>
