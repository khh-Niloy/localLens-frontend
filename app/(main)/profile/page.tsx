...
export default function ProfilePage() {
  ...
  } = useGetProfileQuery({}, { skip: !userData });
  ...
  const addLanguage = () => {
    if (tempLanguage && !formData.language.includes(tempLanguage)) {
      setValue("language", [...formData.language, tempLanguage], {
        shouldDirty: true,
      });
      setTempLanguage("");
    }
  ...
  const addExpertise = () => {
    if (tempExpertise && !formData.expertise.includes(tempExpertise)) {
      setValue("expertise", [...formData.expertise, tempExpertise], {
        shouldDirty: true,
      });
      setTempExpertise("");
    }
  ...
  const removeExpertise = (exp: string) => {
    setValue(
      "expertise",
      formData.expertise.filter((e) => e !== exp),
      { shouldDirty: true }
    );
  };
  ...
  const addTravelPreference = () => {
    if (
      tempTravelPreference &&
      !formData.travelPreferences.includes(tempTravelPreference)
    ) {
      setValue(
        "travelPreferences",
        [...formData.travelPreferences, tempTravelPreference],
        { shouldDirty: true }
      );
  ...
  const removeTravelPreference = (pref: string) => {
    setValue(
      "travelPreferences",
      formData.travelPreferences.filter((p) => p !== pref),
      { shouldDirty: true }
    );
  };
  ...
  const onProfileSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      if (isGuide) {
        if (dirtyFields.expertise)
          formDataToSend.append("expertise", JSON.stringify(data.expertise));
        if (dirtyFields.dailyRate)
          formDataToSend.append("dailyRate", data.dailyRate);
      } else if (isTourist) {
        if (dirtyFields.travelPreferences)
          formDataToSend.append(
            "travelPreferences",
            JSON.stringify(data.travelPreferences)
          );
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 py-6 sm:py-8 md:py-12 px-4 sm:px-6">
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 md:mb-10">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 tracking-tight">
              Profile <span className="text-[#4088FD]">Settings</span>
            </h1>
        ...
        <form
          onSubmit={handleFormSubmit(onProfileSubmit)}
          className="space-y-8"
        >
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 md:gap-8 mb-6 sm:mb-8 p-4 sm:p-6 bg-gray-50/50 rounded-xl sm:rounded-2xl border border-dashed border-gray-200">
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-sm text-gray-500 mb-4 max-w-sm">
                  Recommended: Square image, max 5MB. This photo will be visible
                  to other users.
                </p>
                <input
                  id="profile-upload"
            ...
            <>
                ...
                <div className="space-y-6">
                  <div className="max-w-md">
                    ...
                    <div className="flex gap-2">
                      <Select
                        value={tempExpertise}
                        onValueChange={setTempExpertise}
                      >
                        <SelectTrigger className="rounded-xl border border-gray-100 bg-gray-50 h-12 outline-none focus:ring-4 focus:ring-blue-50 focus:border-[#4088FD] transition-all">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {formData.expertise.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-400 font-medium italic">
                        Share your expertise to attract more tourists!
                      </p>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                <div className="max-w-md">
                  ...
                  <div className="relative group">
                    <span className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-900 font-black">
                      TK
                    </span>
                    <input
                      type="number"
                  ...
                  <p className="text-xs text-gray-400 mt-3 font-medium leading-relaxed">
                    Set a competitive daily rate to attract more bookings. You
                    can update this anytime based on demand.
                  </p>
                </div>
              </motion.div>
            </>
            ...
            >
              ...
              <div className="space-y-6">
                <div className="max-w-md">
                  ...
                  <div className="flex gap-2">
                    <Select
                      value={tempTravelPreference}
                      onValueChange={setTempTravelPreference}
                    >
                      <SelectTrigger className="rounded-xl border border-gray-100 bg-gray-50 h-12 outline-none focus:ring-4 focus:ring-blue-50 focus:border-[#4088FD] transition-all">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {formData.travelPreferences.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-400 font-medium italic">
                      Tell guides what you love to get better recommendations!
                    </p>
                  )}
                </div>
              </div>
